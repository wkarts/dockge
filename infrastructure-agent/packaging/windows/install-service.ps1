...
function Write-Title {
    ...
    Write-Host '╚══════════════════════════════════════════════════════════════════╝' -ForegroundColor Cyan
    Write-Host 'Agente genérico para PIGE360, Connect|API, ERP, Scheduler e outros.' -ForegroundColor DarkGray
    Write-Host
}
...
function Install-Agent {
    ...
    Write-Info 'Preparando diretórios persistentes...'
    New-Item -ItemType Directory -Force -Path $InstallDir,$ConfigDir,(Join-Path $ConfigDir 'secrets') | Out-Null
    ...
    sc.exe create $ServiceName binPath= $binPath start= auto DisplayName= 'Infrastructure Agent' | Out-Null
    sc.exe description $ServiceName 'Generic REST infrastructure enrollment and deployment reconciliation agent' | Out-Null
    Write-Ok "Agent instalado em $InstallDir"
    Write-Ok 'Serviço do Windows registrado para inicialização automática.'
}
function Configure-Agent {
    ...
    $dockgeCredential = Get-PlainText (Read-Host 'Credencial da Dockge API (Enter se ainda não configurada)' -AsSecureString)
    try {
        ...
        Write-Ok 'Configuração criada; credenciais separadas do agent.json.'
        if (-not [string]::IsNullOrWhiteSpace($enrollment)) {
            Write-Info 'Realizando enrollment...'
            & $agent --config $ConfigFile enroll
            if ($LASTEXITCODE -eq 0) { Write-Ok 'Enrollment concluído.' } else { Write-Warn 'Enrollment pendente; configuração foi preservada.' }
        }
    } finally {
...
function Uninstall-Agent {
    Require-Admin
    Write-Warn 'A desinstalação preservará configuração e dados persistentes.'
...
while ($true) {
    ...
    Write-Host '  3) Configurar ou trocar Control Plane'
